...
{
    public class BoletinEliminadoHostedService : IHostedService, IDisposable
    {
        private readonly IConsumerApplication _consumerApplication;
        private readonly IHubContext<PublishCoreHubs> _hubContext;
        private Timer _timer;

        public BoletinEliminadoHostedService(IConsumerApplication consumerApplication)
        {
        ...
        {
            var message = _consumerApplication.StartConsuming("boletinEliminado", "BoletinEliminado");

            if (message != null)
            {
                _hubContext.Clients.All.SendAsync("BoletinEliminado", message);
            }
        }
