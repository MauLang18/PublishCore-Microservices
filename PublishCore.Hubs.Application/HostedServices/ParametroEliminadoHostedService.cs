...
{
    public class ParametroEliminadoHostedService : IHostedService, IDisposable
    {
        private readonly IConsumerApplication _consumerApplication;
        private readonly IHubContext<PublishCoreHubs> _hubContext;
        private Timer _timer;

        public ParametroEliminadoHostedService(IConsumerApplication consumerApplication, IHubContext<PublishCoreHubs> hubContext)
        {
        ...
        {
            var message = _consumerApplication.StartConsuming("parametroEliminado", "ParametroEliminado");

            if (message != null)
            {
                _hubContext.Clients.All.SendAsync("ParametroEliminado", message);
            }
        }
