...
{
    public class UsuarioEliminadoHostedService : IHostedService, IDisposable
    {
        private readonly IConsumerApplication _consumerApplication;
        private readonly IHubContext<PublishCoreHubs> _hubContext;
        private Timer _timer;

        public UsuarioEliminadoHostedService(IConsumerApplication consumerApplication, IHubContext<PublishCoreHubs> hubContext)
        {
        ...
        {
            var message = _consumerApplication.StartConsuming("usuarioEliminado", "UsuarioEliminado");

            if (message != null)
            {
                _hubContext.Clients.All.SendAsync("UsuarioEliminado", message);
            }
        }
