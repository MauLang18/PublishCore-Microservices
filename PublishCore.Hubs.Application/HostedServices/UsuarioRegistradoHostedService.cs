...
{
    public class UsuarioRegistradoHostedService : IHostedService, IDisposable
    {
        private readonly IConsumerApplication _consumerApplication;
        private readonly IHubContext<PublishCoreHubs> _hubContext;
        private Timer _timer;

        public UsuarioRegistradoHostedService(IConsumerApplication consumerApplication, IHubContext<PublishCoreHubs> hubContext)
        {
        ...
        {
            var message = _consumerApplication.StartConsuming("usuarioRegistrado", "UsuarioRegistrado");

            if (message != null)
            {
                _hubContext.Clients.All.SendAsync("UsuarioRegistrado", message);
            }
        }
