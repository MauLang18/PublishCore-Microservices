...
{
    public class BannerPrincipalActualizadoHostedService : IHostedService, IDisposable
    {
        private readonly IConsumerApplication _consumerApplication;
        private readonly IHubContext<PublishCoreHubs> _hubContext;
        private Timer _timer;

        public BannerPrincipalActualizadoHostedService(IConsumerApplication consumerApplication, IHubContext<PublishCoreHubs> hubContext)
        {
        ...
        {
            var message = _consumerApplication.StartConsuming("bannerActualizado", "BannerActualizado");

            if (message != null)
            {
                _hubContext.Clients.All.SendAsync("BannerActualizado", message);
            }
        }
